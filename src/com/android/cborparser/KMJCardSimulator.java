...
public class KMJCardSimulator implements KMSEProvider {
    ...
    switch (alg) {
      case KMType.RSA:
        RSAPrivateKey rsaKey =
            createRsaKey(
                pubModBuf, pubModStart, pubModLength, privKeyBuf, privKeyStart, privKeyLength);
        break;
      case KMType.EC:
  ...
  public HMACKey createHMACKey(byte[] secretBuffer, short secretOff, short secretLength) {
    ...
    key =
        (HMACKey)
            KeyBuilder.buildKey(
                KeyBuilder.TYPE_HMAC, KeyBuilder.LENGTH_HMAC_SHA_256_BLOCK_64, false);
  ...
  public short aesGCMEncrypt(
    ...
    Util.arrayCopyNonAtomic(
        outputBuf, (short) 0, encSecret, encSecretStart, (short) (len - AES_GCM_TAG_LENGTH));
  ...
  public HMACKey cmacKdf(
    ...
    final byte[] L = {
      0, 0, 1, 0
    }; // [L] 256 bits - hardcoded 32 bits as per reference impl in keymaster.
  ...
  public short hmacKDF(
    ...
    return hmacSign(
        keyData, (short) 0, keyLen, data, dataStart, dataLength, signature, signatureStart);
  }
  ...
  private short hkdfExpand(
    ...
    for (short i = 0; i < n; i++) {
      cnt[0]++;
      hmacSignature.init(hmacKey, Signature.MODE_SIGN);
      if (i != 0) hmacSignature.update(previousOutput, (short) 0, (short) 32);
        ...
        if (blockMode != KMType.GCM) {
          KMCipher cipher =
              createSymmetricCipher(
                  alg, purpose, blockMode, padding, keyBuf, keyStart, keyLength, ivBuf, ivStart,
                  ivLength);
          return new KMOperationImpl(cipher);
        } else {
          KMCipher aesGcm =
              createAesGcmCipher(
                  purpose, macLength, keyBuf, keyStart, keyLength, ivBuf, ivStart, ivLength);
          return new KMOperationImpl(aesGcm);
        }
      case KMType.HMAC:
        Signature signerVerifier =
            createHmacSignerVerifier(purpose, digest, keyBuf, keyStart, keyLength);
  ...
  public KMOperation initSymmetricOperation(
    ...
    switch (alg) {
      case KMType.HMAC:
        Signature signerVerifier =
            createHmacSignerVerifier(purpose, digest, keyData, (short) 0, keyLen);
  ...
  public KMOperation initTrustedConfirmationSymmetricOperation(KMKey computedHmacKey) {
    ...
    Signature signerVerifier =
        createHmacSignerVerifier(KMType.VERIFY, KMType.SHA2_256, key.hmacKey);
    return new KMOperationImpl(signerVerifier);
  }
  ...
  public KMOperation initAsymmetricOperation(
    ...
    if (alg == KMType.RSA) {
      ...
      switch (purpose) {
        case KMType.SIGN:
          Signature signer = createEcSigner(digest, privKeyBuf, privKeyStart, privKeyLength);
          return new KMOperationImpl(signer);
        case KMType.AGREE_KEY:
          KeyAgreement keyAgreement = createKeyAgreement(privKeyBuf, privKeyStart, privKeyLength);
  ...
  public KMCipher createRsaDecipher(
      short padding,
      short digest,
      byte[] secret,
      short secretStart,
  ...
  private KMCipher createRsaOAEP256Cipher(
      byte mode,
      byte mgfDigest,
      byte[] secret,
      short secretStart,
    ...
    try {
      ...
      OAEPParameterSpec oaepSpec =
          new OAEPParameterSpec(
              "SHA-256", "MGF1", getMGF1ParamSpec(mgfDigest), PSource.PSpecified.DEFAULT);
  ...
  public Signature createRsaSigner(
      short digest,
      short padding,
      byte[] secret,
      short secretStart,
    ...
    if (padding == KMType.PADDING_NONE
      ...
      return createNoDigestSigner(
          padding, secret, secretStart, secretLength, modBuffer, modOff, modLength);
    }
  ...
  private Signature createNoDigestSigner(
      short padding,
      byte[] secret,
      short secretStart,
      short secretLength,
    ...
    key.setModulus(modBuffer, modOff, modLength);
    rsaCipher.init(key, Cipher.MODE_DECRYPT);
    KMRsa2048NoDigestSignature inst =
        new KMRsa2048NoDigestSignature(rsaCipher, (byte) padding, modBuffer, modOff, modLength);
    return inst;
  }

  public KeyAgreement createKeyAgreement(byte[] secret, short secretStart, short secretLength) {
    ECPrivateKey key =
        (ECPrivateKey)
  ...
  public Signature createEcSigner(
    ...
    if (digest == KMType.DIGEST_NONE) {
      ecSigner =
          new KMEcdsa256NoDigestSignature(Signature.MODE_SIGN, secret, secretStart, secretLength);
    } else {
      ECPrivateKey key =
          (ECPrivateKey)
              KeyBuilder.buildKey(
  ...
  public KMCipher createSymmetricCipher(
    ...
    switch (cipherAlg) {
      ...
      case Cipher.ALG_AES_CTR: // uses SUNJCE
        return createAesCtrCipherNoPad(
            purpose, secret, secretStart, secretLength, ivBuffer, ivStart, ivLength);
  ...
  private KMCipher createAesCtrCipherNoPad(
      short mode,
      byte[] secret,
      short secretStart,
      short secretLength,
  ...
  public Signature createHmacSignerVerifier(
      short purpose, short digest, byte[] secret, short secretStart, short secretLength) {
    ...
    HMACKey key =
        (HMACKey) KeyBuilder.buildKey(KeyBuilder.TYPE_HMAC, (short) (secretLength * 8), false);
  ...
  public KMCipher createAesGcmCipher(
      short mode,
      short tagLen,
      byte[] secret,
      short secretStart,
  ...
  public void newRandomNumber(byte[] num, short startOff, short length) {
    ...
    while (length > 0) {
      ...
      aesRngCipher.doFinal(countBuf, (short) 0, AES_BLOCK_SIZE, randBuf, (short) 0);
  ...
  public short signWithDeviceUniqueKey(
      ...
      short outputDataStart) {
    ECPrivateKey key =
        (ECPrivateKey) ((KMECDeviceUniqueKey) deviceUniqueKey).ecKeyPair.getPrivate();
